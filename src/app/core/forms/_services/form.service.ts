...
export class FormService {
	...
	createForm(eForm: EForm): Observable<EForm> {
		const httpHeaders = new HttpHeaders();
		// Note: Add headers if needed (tokens/bearer)
		httpHeaders.set('Content-Type', 'application/json');
		return this.http.post<EForm>(API_FORMS_URL, eForm, { headers: httpHeaders });
	}
	...
	getFormById(eFormId: number): Observable<EForm> {
		if (!eFormId) {
			return of(null);
		}

		return this.http.get<EForm>(API_FORMS_URL + `/${eFormId}`);
	}
